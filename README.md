...
src/server/
  ...
  audit-log.ts    the audit sink: stdout plus the phi_audit_events table (ADR 36)
...
| `/api/v1/[[...path]]` | the partner REST API (ADR 37); `/api/v1/openapi.json` and `/api/v1/docs` are its reference; 404 unless `PARTNER_API_ENABLED=true` |
