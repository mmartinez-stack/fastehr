...
describe('PHI audit', () => {
  it('records an allowed access', async () => {
    await expect(callWith({ id: 'user-1', roles: ['clinician'], offices: ['Sylmar'] })).resolves.toBe('Lovelace, Ada')
  ...
  it('records an unauthorized attempt as denied, with the actor that made it', async () => {
    await expect(callWith({ id: 'user-2', roles: [], offices: ['Sylmar'] })).rejects.toThrow('FORBIDDEN')
  ...
  it('never records the procedure input', async () => {
    await callWith({ id: 'user-1', roles: ['clinician'], offices: ['Sylmar'] })
