...
/** One bucket per distinct identifier set a client asks about, keyed by a hash so the identifiers are not held in memory. */
function identifierKey(clientId: string, body: Record<string, string | undefined>): string {
...
export const lookupPatients: OperationHandler<typeof lookupPatientsOperation> = async ({ ctx, body }) => {
  const perIdentifier = ctx.rateLimiter.take(
    identifierKey(ctx.actor.clientId, body),
...
export const verifyPatient: OperationHandler<typeof verifyPatientOperation> = async ({ ctx, params, body }) => {
  ...
  await assertNotLockedOut(ctx, { clientId, patientId })
  ...
  await ctx.db.verifications.recordAttempt({ apiClientId: clientId, patientId, succeeded: matched, ipAddress: ctx.ipAddress })
  if (!matched) throw new PartnerApiError('verification_failed')

  const issued = await issueVerificationToken(ctx, { clientId, patientId })
